...
async function ensureUserExperienceFeedbackTable() {
    await query(`
...
async function bootstrap() {
    ...
    try {
        ...
        await ensureUserExperienceFeedbackTable();
        ...
        await query(`
            DELETE FROM tier_rules WHERE tier IS NULL OR tier NOT IN ('normal', 'deep');
            DELETE FROM tier_rules a USING tier_rules b WHERE a.ctid < b.ctid AND a.tier = b.tier;
        `);
        await query(`
            CREATE UNIQUE INDEX IF NOT EXISTS idx_tier_rules_tier ON tier_rules (tier);
        `).catch(err => console.warn('[dbBootstrap] Warning: failed to ensure unique index on tier_rules:', err.message));
        ...
        await query(`
            ...
            VALUES
                ...
                ('user_experience_feedback', 'active', true, 'frontend', 'Public homepage feedback with manual approval workflow.')
